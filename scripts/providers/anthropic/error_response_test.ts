...
const baselineModel =
  process.env.ERROR_MODEL_OVERRIDE || "claude-sonnet-4-5-20250929";
...
async function buildEnvelope(
  ...
  return {
    ...
    request: {
      url: requestMeta?.url ?? "https://api.anthropic.com/v1/messages",
    ...
    try {
      const requestOpts =
        scenario.apiKeyOverride !== undefined
          ...
          : undefined;
      const response = await getAnthropicResponse(model, {
        ...payload,
        model,
      }, requestOpts);
      ...
      const envelope = await buildEnvelope(
        ...
        },
        {
          headers: {
            ...
            "X-Api-Key": scenario.apiKeyOverride ?? process.env.ANTHROPIC_API_KEY ?? "",
          },
          body: {
            ...payload,
            model,
