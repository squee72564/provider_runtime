...
const baselineModel = process.env.ERROR_MODEL_OVERRIDE || "openai/gpt-5-mini";
...
const scenarios: ScenarioDef[] = [
  ...
  },
  {
    ...
    modelOverride: "openai/this-model-does-not-exist",
  },
  {
...
async function buildEnvelope(
  ...
  return {
    ...
    request: {
      url: requestMeta?.url ?? "https://openrouter.ai/api/v1/chat/completions",
    ...
    try {
      const requestOpts =
        scenario.apiKeyOverride !== undefined
          ...
          : undefined;
      const response = await getOpenRouterResponse(model, {
        ...payload,
        model,
      }, requestOpts);
      ...
      const envelope = await buildEnvelope(
        ...
        },
        {
          headers: {
            ...
            Authorization: `Bearer ${scenario.apiKeyOverride ?? process.env.OPENROUTER_API_KEY ?? ""}`,
          },
          body: {
            ...payload,
            model,
